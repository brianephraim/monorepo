...
const configPath = `${__dirnameBeforeCompiled}/jestConfig.js`;
// const configBasePath = `${__dirnameBeforeCompiled}/jestConfigBase.js`;
// qwer();
...
export default (testdevenv, watch = true, testPathPattern) => {
  ...
  if (testdevenv) {
    ...
    // commandBody = `CI=true ${process.cwd()}/node_modules/.bin/jest ${__dirnameBeforeCompiled}/testdevenv.js  --config=${configBasePath}`;
    commandBody = `CI=true ${process.cwd()}/node_modules/.bin/mocha-webpack --colors --require babel-core/register --webpack-config ${path.resolve(__dirnameBeforeCompiled, '../core/webpackConfigMocha.js')} \"${path.resolve(__dirnameBeforeCompiled, './testdevenv2.js')}\" --require source-map-support/register`;
  }
  const args = [
    '-c',
    commandBody,
