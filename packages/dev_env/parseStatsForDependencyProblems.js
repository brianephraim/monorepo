...
function parseStatsForDependencyProblems(stats, packagesDir, shouldShowProblemsInConsole = true) {
  ...
  stats.compilation.modules.forEach((module) => {
    ...
    if (
      ...
      const packageDotJsonContent = packageDotJsonCache[packageDotJsonPath] || ensureReadJsonSync(packageDotJsonPath);
      packageDotJsonCache[packageDotJsonPath] = packageDotJsonContent;

      const packageSomehowPublic = !packageDotJsonContent.private || !packageDotJsonContent.privateFromGithub;
      ...
      if (Object.keys(packageDotJsonContent).length && packageSomehowPublic) {
        let depProblems = findDependenciesProblems(module.dependencies, packageDotJsonContent);
        if (depProblems) {
